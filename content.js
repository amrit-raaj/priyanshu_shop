...
function dynamicClothingSection(ob) {
  ...
  icon.onclick = function () {
    ...
    let counter = 0;
    if (document.cookie.indexOf(',counter=') >= 0) {
      if (document.cookie.split(',')[1].split('=')[1] == "NaN") {
        counter = counter + 1;
        order=ob.id+" ";
      }
      else {
...
let k = document.getElementsByClassName("1")[1];
let m = document.getElementsByClassName("2")[0];
    ...
    if (this.status == 200) {
      ...
      for (let i = 0; i < contentTitle.length; i++) {
        ...
        if (contentTitle[i].isaccessory) {
          ...
          m.appendChild(dynamicClothingSection(contentTitle[i])
          );
        }
      }
    } else {
